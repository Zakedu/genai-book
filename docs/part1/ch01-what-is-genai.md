...
| Gemini 3.5 Pro | Google | 비공개[^3] | 2026.03 Preview, 차세대 멀티모달 |
...
!!! warning "학습 데이터의 한계"
    1. **학습 시점 이후의 정보는 제한적**: 최신 모델은 웹 검색 기능으로 실시간 정보를 보완하지만, 학습 데이터 자체의 시점 한계는 여전히 존재합니다.
...
| **실시간 정보 한계** | 웹 검색으로 보완하나 완벽하지 않음 | 최신 뉴스, 주가 등은 검색 기반 답변 |
...
| **멀티모달** | 텍스트+이미지 | 텍스트+이미지 | GPT-5.6, Claude Opus 4.8, Gemini 3.5 Pro |

### 멀티모달 AI의 등장

!!! tip "멀티모달(Multimodal)"
    텍스트, 이미지, 음성 등 **여러 형태의 데이터를 동시에 이해하고 생성**할 수 있는 AI입니다. 2026년 현재 모든 주요 AI에 탑재된 **업계 표준** 기능입니다.
...
!!! abstract "이 챕터의 핵심 포인트"
    ...
    5. **한계**: 환각(사실 오류), 실시간 정보 한계, 수학적 추론 한계
...
[^3]: Google. (2026). *Gemini 3.5 Pro Documentation*. https://ai.google.dev/docs

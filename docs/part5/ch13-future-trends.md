...
여러 형태의 정보(텍스트, 이미지, 오디오, 비디오)를 동시에 이해하고 생성하는 능력입니다. 2026년 현재 멀티모달 AI는 더 이상 미래 기술이 아닌 **업계 표준**입니다. GPT-5.6, Claude Opus 4.8 (인라인 시각화 지원), Gemini 3.5 Pro 등 모든 주요 모델이 네이티브 멀티모달을 지원합니다.

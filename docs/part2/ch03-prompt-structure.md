...
프로그래머가 코드를 디버깅하듯, 프롬프트도 **의도적으로 문제를 찾고 고치는 훈련**이 필요합니다. Bad Prompt Makeover는 Anthropic Academy에서 사용하는 실습 방식으로, 의도적으로 나쁜 프롬프트를 제시하고 학습자가 **개선점을 찾아 리팩토링**하는 활동입니다.

!!! tip "왜 이 활동이 효과적인가"
    - 좋은 프롬프트를 *쓰는 것*보다 나쁜 프롬프트의 *문제점을 찾는 것*이 학습 효과가 높다
...
!!! abstract "이 챕터의 핵심 포인트"
    ...
    5. **Bad Prompt Makeover**: 나쁜 프롬프트를 디버깅하듯 분석하고 리팩토링하는 훈련
    6. **체크리스트 활용**: 작성 후 모호함 여부 점검
